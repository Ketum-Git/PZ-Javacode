...
public class InputScript extends CraftRecipe.IOScript {
    ...
    protected void OnPostWorldDictionaryInit() throws Exception {
        if (this.createToItemScript != null
            ...
            for (String sx : this.loadedFluids) {
                Fluid fluid = Fluid.Get(sx);
                if (fluid == null) {
                    throw new Exception("Fluid not found: " + sx + ". line: " + this.originalLine);
                }

                this.fluidFilter.add(fluid);
            }

            for (String sx : this.categories) {
                FluidCategory fc = FluidCategory.valueOf(sx);
                if (fc == null) {
                    throw new Exception("FluidCategory not found: " + sx + ". line: " + this.originalLine);
                }
            ...
            for (String sx : this.loadedEnergies) {
                Energy energy = Energy.Get(sx);
                if (energy == null) {
                    throw new Exception("Energy not found: " + sx + ". line: " + this.originalLine);
                }
