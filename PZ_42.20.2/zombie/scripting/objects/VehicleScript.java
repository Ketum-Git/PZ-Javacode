...
public final class VehicleScript extends BaseScriptObject implements IModelAttachmentOwner {
    ...
    private void initCrawlOffsets() {
        ...
        for (int ix = 0; ix < this.getCrawlThroughWheel(); ix++) {
            VehicleScript.Wheel wheel = this.getCrawlThroughWheel(ix);
            if (wheel.id.contains("Left")) {
                this.initCrawlOffsets(wheel);
            }
        }
        ...
        for (int ixx = 0; ixx < this.crawlOffsets.size(); ixx++) {
            this.crawlOffsets.set(ixx, (this.extents.z / 2.0F + 0.15F + this.crawlOffsets.get(ixx) - this.centerOfMassOffset.z) / polyLength);
        }

        this.crawlOffsets.sort();

        for (int ixx = 0; ixx < this.crawlOffsets.size(); ixx++) {
            float o1 = this.crawlOffsets.get(ixx);

            for (int j = ixx + 1; j < this.crawlOffsets.size(); j++) {
