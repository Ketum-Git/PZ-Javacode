                ...
                if (this.processIsoBarricadeOpposite(owner, weapon, hitInfo) && hitObject != null) {
                    ...
                    if (owner.isAimAtFloor() && !weapon.isRanged() && owner.isNpc()) {
                        ...
                        if (ownerPlayer == null || ownerPlayer.isLocalPlayer()) {
                            if (!StringUtils.isNullOrEmpty(weapon.getHitFloorSound())) {
                                ...
                                owner.getEmitter().stopSoundByName(weapon.getSwingSound());
                                ParameterMeleeHitSurface.Material materialx = ParameterMeleeHitSurface.Material.Body;
                                if (ownerPlayer != null) {
                                    ownerPlayer.setMeleeHitSurface(materialx);
                                }

                                zombieHitSound = owner.getEmitter().playSoundImpl(weapon.getZombieHitSound(), null);
                                if (GameClient.client) {
                                    INetworkPacket.send(
                                        GameClient.connection, PacketTypes.PacketType.PlaySound, weapon.getZombieHitSound(), (byte)0, owner, materialx
                                    );
                                }
                            }
                        }
                    ...
                    if (!owner.getAttackVars().aimAtFloor
                        ...
                        if (weapon.isRanged()) {
                            ...
                            ParameterBulletHitSurface.Material materialxx = hitHead > 0
                                ? ParameterBulletHitSurface.Material.Flesh_Hollow
                                : ParameterBulletHitSurface.Material.Flesh;
                            isoGameCharacter.getEmitter().setParameterValueByName(zombieHitSound, "BulletHitSurface", materialxx.label);
                            if (GameClient.client) {
                                INetworkPacket.send(
                                    GameClient.connection, PacketTypes.PacketType.PlaySound, weapon.getZombieHitSound(), (byte)0, isoGameCharacter, materialxx
                                );
                            }
                        } else {
                            owner.getEmitter().stopSoundByName(weapon.getSwingSound());
                            ParameterMeleeHitSurface.Material materialxx = ParameterMeleeHitSurface.Material.Body;
                            if (ownerPlayer != null) {
                                ownerPlayer.setMeleeHitSurface(materialxx);
                            }

                            zombieHitSound = owner.getEmitter().playSoundImpl(weapon.getZombieHitSound(), null);
                            if (GameClient.client) {
                                INetworkPacket.send(
                                    GameClient.connection, PacketTypes.PacketType.PlaySound, weapon.getZombieHitSound(), (byte)0, owner, materialxx
                                );
                            }
                        }
                    }
                    ...
                    if (hitVehicle == null && hitObject.getSquare() != null && owner.getSquare() != null) {
                        ...
                        if (ownerPlayer.isLocalPlayer() || owner.isNpc()) {
                            ...
                            if (hitObject instanceof IsoGameCharacter isoGameCharacterx) {
                                this.applyMeleeEnduranceLoss(owner, isoGameCharacterx, weapon, hitDamage);
                            }
    ...
    private HitInfo calcValidTarget(IsoLivingCharacter owner, HandWeapon weapon, IsoMovingObject mov, float range) {
        if (mov == owner) {
            ...
            if (mov.getCurrentSquare() == null) {
                ...
                return null;
            } else if (!isoGameCharacter.isAnimal() && !checkPVP(owner, mov, weapon.isMelee())) {
                return null;
