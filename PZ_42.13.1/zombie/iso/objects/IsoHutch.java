    ...
    private void releaseAnimal(IsoGridSquare animalSq, IsoAnimal animal) {
        ...
        if (animalSq != null) {
            if (!GameClient.client) {
                IsoAnimal newAnimal = new IsoAnimal(animalSq.getCell(), animalSq.x, animalSq.y, animalSq.z, animal.getAnimalType(), animal.getBreed());
                newAnimal.copyFrom(animal);
                animal.hutch = null;
                animal.getData().enterHutchTimerAfterDestroy = 300;
                newAnimal.hutch = null;
                newAnimal.addToWorld();
                newAnimal.setStateEventDelayTimer(0.0F);
                if (GameServer.server) {
                    AnimalInstanceManager.getInstance().remove(animal);
                    AnimalSynchronizationManager.getInstance().delete(animal.getOnlineID());
                }
            }
