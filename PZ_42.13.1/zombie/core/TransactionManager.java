        ...
        if (destination != null
            ...
            for (Transaction transaction : transactions) {
                ...
                if (transaction.itemId != -1
                    ...
                    if (item != null) {
                        destinationWeightInTransaction += item.getActualWeight();
                    }
                }
            }
            ...
            if (itemId != -1 && source != null) {
                ...
                if (destination != null && "floor".equals(destination.getType()) && source.getCharacter() != null) {
                    IsoGridSquare dropSquare = getNotFullFloorSquare(source.getCharacter(), item, destination);
                    if (dropSquare == null) {
                ...
                if (destination != null && !destination.hasRoomFor(player, item.getActualWeight() + destinationWeightInTransaction)) {
            ...
            if (worldItem != null && destination != null && !destination.isItemAllowed(worldItem)) {
                ...
                && destination != null
                && !destination.hasRoomFor(player, worldItem.getActualWeight() + destinationWeightInTransaction)) {
                DebugLog.Objects.noise("Inconsistent: destination container is not contain enough space for the item (%s)", description);
                return 1;
            } else {
                return 0;
